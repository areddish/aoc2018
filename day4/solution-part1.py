...
with open("input.txt", "rt") as file:
    ...
    max_minutes = 0
    for guard in asleep.keys():
        if (minutes[guard][0] >= max_minutes):
            print("MAX", guard, minutes[guard][0])
            max_minutes = minutes[guard][0]
            guard_slept_the_most = guard

    print("Guard who slept the most:", guard_slept_the_most)
    print(minutes[guard_slept_the_most][1])
    minute_asleep_the_most = max(minutes[guard_slept_the_most][1])
    print(minute_asleep_the_most,
          minutes[guard_slept_the_most][1].index(minute_asleep_the_most))
